...
```{r}
starbucks %>% 
  select(calories:caffeine_mg)
```

2) How many different drinks are represented in the data?
```{r}
starbucks %>% 
  summarize(n_distinct(product_name))
```

3) What are the quantiles and mean values for caffeine in the drinks represented? 
```{r}
starbucks %>% 
  summarize(quant25 = quantile(caffeine_mg, 0.25, na.rm = T),
            mean = mean(caffeine_mg, na.rm = T),
            quant75 = quantile(caffeine_mg, 0.75, na.rm = T))
```
...
```{r}
starbucks <- starbucks %>% 
  mutate(across(contains("_g"), as.numeric))

starbucks %>% 
  mutate(across(contains("_g"),function(x) {x * 1000}))
```
...
starbucks %>% 
  rowwise() %>% 
  mutate(good_stuff = mean(c(calories, sugar_g, caffeine_mg, sodium_mg, na.rm = T)))
```
...
starbucks %>% 
  group_by(size) %>% 
  slice_max(sugar_g, n=3, with_ties = F)
```
