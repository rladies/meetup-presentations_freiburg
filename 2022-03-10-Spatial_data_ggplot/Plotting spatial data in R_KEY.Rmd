...
BONUS: Create a choropleth map showing the preferred alcoholic beverage in different European countries (see below).\ Hint: You can calculate the shares of each serving (e.g. beer_servings/total_servings) and use the base-R function max.col to find the column with the highest value.
...
#we then need to calculate the sum of all servings and exclude zeros in total_servings (to avoid error division by zero)

alcohol<-alcohol %>% 
  mutate(total_servings=beer_servings+spirit_servings+wine_servings) %>% 
  filter(total_servings!=0)

#we then calculate the share of beer, wine and spirits of total servings

alcohol<-alcohol %>% 
  mutate(share_beer=beer_servings/total_servings) %>% 
  mutate(share_wine=wine_servings/total_servings) %>%
  mutate(share_spirit=spirit_servings/total_servings)  
  
#we then identify the column with the maximum value and rename the column number to beer, spirit, wine

alcohol<-alcohol %>% 
  mutate(preferred_col=max.col(alcohol[, 7:9])) %>% 
  mutate(preferred_alcohol=ifelse(preferred_col==1, "beer", ifelse(preferred_col==2, "spirit", ifelse(preferred_col==3, "wine", "other"))))
...
europe_map_spirits<-ggplot(europe_combined, aes(x=long, y=lat, group=group))+ 
  ...
  scale_fill_manual(values=c("#ecca00", "#42e9f5", "#ab273e"), na.value="#dfdbd8")+
